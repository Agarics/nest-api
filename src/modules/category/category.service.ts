...
export class CategoryService {
  ...
  private async aggregateArticleCount(
    publicOnly: boolean,
  ...
  public async getAllCategories(options: {
    aggregatePublicOnly: boolean;
    ...
    const allCategories = await this.categoryModel
      ...
      .exec();
    return await this.aggregateArticleCount(
      options.aggregatePublicOnly,
      allCategories,
    );
  }
  ...
  public async paginator(
    ...
    const categories = await this.categoryModel.paginate(query, {
      ...options,
      lean: true,
    });
    const documents = await this.aggregateArticleCount(
      publicOnly,
  ...
  public getDetailBySlug(slug: string): Promise<MongooseDoc<Category>> {
    return this.categoryModel
      ...
      .exec()
      .then(
        (result) => result || Promise.reject(`Category '${slug}' not found`),
      );
  }
      ...
      (function findCateItem(id) {
        findById(id)
          .then((category) => {
            ...
            const hasParent =
              parentId && parentId.toString() !== category._id.toString();
  ...
  // update category
  public async update(
    categoryId: MongooseId,
    newCategory: Category,
  ): Promise<MongooseDoc<Category>> {
    const existedCategory = await this.categoryModel
      .findOne({ slug: newCategory.slug })
      .exec();
  ...
  public async delete(categoryId: MongooseId) {
    const category = await this.categoryModel
      .findByIdAndDelete(categoryId, null)
      .exec();
    if (!category) {
      throw `Category '${categoryId}' not found`;
    }
  ...
  public async batchDelete(categoryIds: MongooseId[]): Promise<any> {
    // SEO remove
    const categories = await this.categoryModel
