...
import { increaseTodayViewsCount } from '@/modules/expansion/expansion.helper';
import { ArchiveService } from '@/modules/archive/archive.service';
import { CategoryService } from '@/modules/category/category.service';
import { TagService } from '@/modules/tag/tag.service';
import { PublishState } from '@/constants/biz.constant';
import { NULL } from '@/constants/value.constant';
import {
  MongooseModel,
  MongooseDoc,
  MongooseId,
...
export class ArticleService {
  ...
  public getDetailByObjectId(
    articleId: MongooseId,
    ...
    return this.articleModel
      ...
      .then(
        (result) =>
          result || Promise.reject(`Article '${articleId}' not found`),
      );
  }
  ...
  public getDetailByNumberIdOrSlug({
    ...
    return this.articleModel
      .findOne(
        publicOnly ? { ...params, ...ARTICLE_LIST_QUERY_GUEST_FILTER } : params,
      )
      .populate(populate ? ARTICLE_FULL_QUERY_REF_POPULATE : [])
      .exec()
      .then(
        (result) => result || Promise.reject(`Article '${idOrSlug}' not found`),
      );
  }

  // get article detail for guest user
  public async getFullDetailForGuest(
  ...
  public async create(newArticle: Article): Promise<MongooseDoc<Article>> {
    if (newArticle.slug) {
      const existedArticle = await this.articleModel
        .findOne({ slug: newArticle.slug })
        .exec();
      if (existedArticle) {
  ...
  public async delete(articleId: MongooseId) {
    const article = await this.articleModel
      .findByIdAndDelete(articleId, null)
      .exec();
    if (!article) {
      throw `Article '${articleId}' not found`;
    }
  ...
  public async batchDelete(articleIds: MongooseId[]): Promise<any> {
    const articles = await this.articleModel
  ...
  public async getTotalCount(publicOnly: boolean): Promise<number> {
    return await this.articleModel
      .countDocuments(publicOnly ? ARTICLE_LIST_QUERY_GUEST_FILTER : {})
      .exec();
  }
  ...
  public getCalendar(publicOnly: boolean, timezone = 'GMT') {
    return this.articleModel
      ...
      .then((calendar) =>
        calendar.map(({ _id, ...r }) => ({ ...r, date: _id })),
      )
      .catch(() =>
        Promise.reject(`Invalid timezone identifier: '${timezone}'`),
      );
  }
  ...
  public async updateMetaComments(articleId: number, commentCount: number) {
    ...
    return this.articleModel
      .updateOne(findParams, patchParams, { timestamps: false })
      .exec();
  }
}
