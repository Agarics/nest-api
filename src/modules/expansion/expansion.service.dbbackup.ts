...
} from '@/processors/helper/helper.service.aws';
import { APP, MONGO_DB, DB_BACKUP } from '@/app.config';
...
export class DBBackupService {
  ...
  private doBackup() {
    return new Promise<UploadResult>((resolve, reject) => {
      ...
      shell.exec(
        `mongodump --quiet --forceTableScan --uri="${MONGO_DB.uri}" --out="backup"`,
        (code, out, err) => {
          if (code === 0) {
          ...
          shell.exec(
            `zip -q -r -P ${DB_BACKUP.password} ${BACKUP_FILE_NAME} ./backup`,
          );
