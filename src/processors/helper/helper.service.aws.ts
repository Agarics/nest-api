...
import { AWS } from '@/app.config';

export {
  StorageClass as AWSStorageClass,
  ServerSideEncryption as AWSServerSideEncryption,
} from '@aws-sdk/client-s3';
...
export class AWSService {
  ...
  public getObjectAttributes(payload: {
    region: string;
    bucket: string;
    key: string;
  }) {
  ...
  public uploadFile(payload: FileUploader): Promise<UploadResult> {
    ...
    return s3Client.send(command).then(() => {
      return this.getObjectAttributes({ region, bucket, key }).then(
        (attributes) => {
          return {
            key,
